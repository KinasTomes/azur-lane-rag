...
# Configuration
DB_NAME = Path(__file__).parent.parent / "azur_lane.db"
DATA_DIR = Path(__file__).parent.parent / "AzurLaneData" / "data"
...
def migrate():
    ...
    for ship_id_str, ship in ships_data.items():
        ...
        # Main ship info
        cursor.execute("""
            INSERT OR REPLACE INTO ships (id, gid, name, global_name, rarity_id, nation_id, hull_id, ship_class, sub_class, release_date, icon, flags)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
